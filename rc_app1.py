...
class RCApp(QMainWindow):
    ...
    def update_status_label(self):
        ...
        if self.joy:
            tokens.append("ctl: OK")
        else:
            tokens.append("ctl: none")

        if self.serial_out:
            try:
                port = self.serial_out.port
                tokens.append(f"out: {port}")
            except Exception:
                tokens.append("out: connected")
        else:
            tokens.append("out: off")

        if self.serial_in:
            try:
                port = self.serial_in.port
                tokens.append(f"in: {port}")
            except Exception:
                tokens.append("in: connected")
        else:
            tokens.append("in: off")

        if not self.sending_enabled:
            mode = "paused"
        elif not self.serial_out:
            mode = "noout"
        elif not self.joy and not self.serial_in:
            mode = "safe"
        else:
            mode = "active"
        tokens.append(f"mode: {mode}")

        status_line = " • ".join(tokens)
        if hasattr(self, 'lbl_status') and self.lbl_status is not None:
            self.lbl_status.setText(status_line)
    ...
    def init_ui(self):
        ...
        self.sl_throttle_limit = QSlider(Qt.Horizontal)
        self.sl_throttle_limit.setRange(10, 100)
        self.sl_throttle_limit.setValue(50)
        ...
        t.addWidget(QLabel("Max throttle (%)"))
        t.addWidget(self.sl_throttle_limit)
    ...
    def update_logic(self):
        ...
        if self.joy:
            try:
                ...
                r2 = (self.joy.get_axis(self.default_R2_axis) + 1) / 2 if self.joy.get_numaxes() > self.default_R2_axis else 0.0
                l2 = (self.joy.get_axis(self.default_L2_axis) + 1) / 2 if self.joy.get_numaxes() > self.default_L2_axis else 0.0
            except Exception:
                raw_steer = 0
                r2 = 0
                l2 = 0
    ...
    def update_gui(self):
        ...
        try:
            ...
            self.lbl_telemetry.setText(
                f"S: {s_val}  T: {t_val}\nYaw: {adj_yaw:.1f}°  Pitch: {adj_pitch:.1f}°  MaxT: {self.sl_throttle_limit.value()}%"
            )
        except Exception:
        ...
        try:
            if self.joy:
                try:
                    ...
                    name = "Controller"
                ctl_text = f"ctl: {name}"
                self.lbl_status.setText(ctl_text)
            else:
                self.update_status_label()
        except Exception:
            pass
